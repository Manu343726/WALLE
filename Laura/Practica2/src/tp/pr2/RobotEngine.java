		...
		do{
			...
			if(!instruction.isValid())
				...
				case PICK:
					...
					if(item == null)
						//No esta ese objeto en ese lugar 
						System.out.println(HASNOTOBJECT + instruction.getId());
					else{
						if(this.itemContainer.getItem(item.getId()) != null){
							//Walle ya tiene el objeto 
							System.out.println(ALREADYHAVEOBJECT + instruction.getId());
							//Lo volvemos a poner en el lugar 
							this.currentPlace.addItem(item);
						}
						else{
							//Lo a�adimos al contenedor de Walle
